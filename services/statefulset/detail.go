...
func (s *Services) DetailStateFulSet(ctx context.Context, req models.DetailQuery) models.Response {
	...
	if err != nil {
		...
		if k8sError.IsNotFound(err) {
			...
			response.Message = fmt.Sprintf("StatefulSet %s 查询出错 %v", req.Name, err)
		}
		return response
	}
