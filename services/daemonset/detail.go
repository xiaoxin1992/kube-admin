...
func (s *Services) DetailDaemonSet(ctx context.Context, req models.DetailQuery) models.Response {
	...
	ds, err := client.AppsV1().DaemonSets(req.Namespace).Get(ctx, req.Name, metav1.GetOptions{})
	if err != nil {
		response.Code = http.StatusNotFound
		if k8sError.IsNotFound(err) {
			response.Message = fmt.Sprintf("daemonSet %s 不存在", req.Name)
		} else {
			response.Message = fmt.Sprintf("daemonSet %s 查询出错 %v", req.Name, err)
		}
		return response
	}
